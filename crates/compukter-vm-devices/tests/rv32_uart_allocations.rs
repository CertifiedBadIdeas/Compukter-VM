...
use compukter_vm::rv32_machine::{
    ...
    DEFAULT_DBT_CODE_ALIGNMENT, DEFAULT_DBT_REGISTER_PROFILE, PLIC_BASE,
};
...
#[test]
fn complete_uart_interrupt_round_trips_allocate_nothing_after_warmup() {
    ...
    for execution in configs() {
        ...
        .unwrap();
        let (uart, _) = builder.add_mmio_device_with_irq(UART_BASE, Uart16550::new());
        ...
        let mut warm_output = [0_u8; 1];
        assert_eq!(
            machine.device_mut(uart).unwrap().drain_tx(&mut warm_output),
            1
        );
        assert_eq!(warm_output, [1]);

        ALLOCATIONS.store(0, Ordering::Relaxed);
        ALLOCATED_BYTES.store(0, Ordering::Relaxed);
        for input in 1_u8..=32 {
            assert_eq!(
                machine
                    .device_mut(uart)
                    .unwrap()
            ...
            assert_eq!(output, [input.wrapping_add(1)]);
        }
        let allocations = ALLOCATIONS.load(Ordering::Relaxed);
        let allocated_bytes = ALLOCATED_BYTES.load(Ordering::Relaxed);

        assert_eq!(allocations, 0, "{execution:?} allocated around UART IRQ");
        assert_eq!(
            allocated_bytes, 0,
            "{execution:?} allocated bytes around UART IRQ"
        );
    }
}

...
SOURCE_ROOT="$ROOT/fixtures/rv32-elf-atomic"
OUTPUT_ELF="$1"
...
for tool in "$RV32_ELF_CLANG" "$RV32_ELF_LLD" "$RV32_ELF_READOBJ" "$RV32_ELF_NM"; do
    if ! command -v "$tool" >/dev/null 2>&1; then
        echo "required RV32 ELF atomic tool is unavailable: $tool" >&2
        exit 2
    fi
done
...
"$RV32_ELF_CLANG" "${common_flags[@]}" -std=c11 -O2 -Wall -Wextra -Werror \
    -c "$SOURCE_ROOT/atomic.c" -o "$BUILD_ROOT/atomic.o"
"$RV32_ELF_LLD" -m elf32lriscv --no-relax --fatal-warnings \
    -T "$SOURCE_ROOT/link.ld" "$BUILD_ROOT/start.o" "$BUILD_ROOT/atomic.o" -o "$OUTPUT_ELF"

if [[ -n "$("$RV32_ELF_NM" --undefined-only "$OUTPUT_ELF")" ]]; then
    echo "RV32 ELF atomic fixture contains undefined symbols" >&2
    exit 1
fi
if "$RV32_ELF_READOBJ" --relocations "$OUTPUT_ELF" | grep -Eq '0x[0-9A-Fa-f]+ R_'; then
    echo "RV32 ELF atomic fixture contains unresolved relocations" >&2
    exit 1
fi

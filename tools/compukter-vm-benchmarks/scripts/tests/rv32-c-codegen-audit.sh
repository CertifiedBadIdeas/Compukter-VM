...
cargo run --manifest-path "$ROOT/Cargo.toml" --release --locked --offline \
    --features dbt-code-audit,dbt-execution-profile -p compukter-vm-benchmarks --bin rv32_c_codegen_audit -- export "$BUILD_DIR"
...
"$RV32_C_CLANG" -O3 -march=native -fno-lto -Wall -Wextra -Werror \
    -c "$ROOT/tools/compukter-vm-benchmarks/assets/rv32-c-comparison/kernel.c" -o "$BUILD_DIR/native-analysis.o"
...
cargo run --manifest-path "$ROOT/Cargo.toml" --release --locked --offline \
    --features dbt-code-audit,dbt-execution-profile -p compukter-vm-benchmarks --bin rv32_c_codegen_audit -- report "$BUILD_DIR"
...
"$RV32_C_LLD" -m elf32lriscv --no-relax --fatal-warnings --defsym=__ck_batch="$AUDIT_BATCH" \
    -T "$ROOT/tools/compukter-vm-benchmarks/assets/rv32-c-comparison/product.ld" "$BUILD_DIR/product-start.o" \
...
cargo build --manifest-path "$ROOT/Cargo.toml" --release --locked --offline \
    --features dbt-code-audit,dbt-execution-profile -p compukter-vm-benchmarks --bin rv32_c_codegen_audit
...
if ! command -v "$RV32_C_PERF" >/dev/null 2>&1; then
    ...
    ! perf_measure rv32-cached-dbt $'CK_RESULT\tee053d58' "$AUDIT_BATCH" \
        "$ROOT/target/release/rv32_c_codegen_audit" \

...
RV32_C_PRODUCT_ELF="$BUILD_DIR/product.elf" \
    cargo test --manifest-path "$ROOT/Cargo.toml" -p compukter-vm-benchmarks \
...
cargo run --manifest-path "$ROOT/Cargo.toml" --release \
    --features wasmtime-comparison \
    -p compukter-vm-benchmarks --bin rv32_c_comparison --locked --offline -- "$BUILD_DIR" 21 \
...
write_dbt_disassembly "rv32-cached-dbt-align-chain-32"

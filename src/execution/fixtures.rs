...
pub(super) fn suspend_value_call_artifact() -> VerifiedArtifact {
    verified_mutated(|artifact| {
        ...
        let mut entry = function(0, 0, vec![i32_type], 0);
        entry.flags = 1;
        entry.block_count = 2;
        let mut child = function(1, 0, vec![i32_type], 2);
        child.flags = 1;
        ...
        let programs = [
            ...
            (FunctionId(0), vec![Instruction::Return { value: 0 }]),
            (
                FunctionId(1),
                vec![
                    Instruction::Const {
...
pub(crate) fn sound_bool_artifact(arguments: &[i32]) -> VerifiedArtifact {
    let mut decoded = crate::decode::records::decode_artifact(
        Arc::from(crate::test_support::minimal_vector_with_string_records(&[
            b"compukter",
            b"sound",
    ...
    let mut constants = arguments.to_vec();
    constants.sort_unstable_by_key(|value| *value as u32);
    constants.dedup();
    decoded.modules[0].constants = constants.iter().copied().map(Constant::I32).collect();
    let function = &mut decoded.modules[0].functions[0];
    function.name = 1;
    ...
    function.register_count = (arguments.len() + 1) as u16;
    function.parameter_count = 0;
    function.values = crate::artifact::scalar_values(
        vec![i32_type; arguments.len()]
            .into_iter()
            .chain(std::iter::once(bool_type))
            .collect(),
    );
    let mut first = arguments
        .iter()
...
pub(super) fn suspend_scalar_capability_artifact() -> VerifiedArtifact {
    verified_mutated(|artifact| {
        let i32_type = primitive(1);
        artifact.header.semantic_features = 0b110;
        artifact.capabilities.push(crate::artifact::Capability {
        ...
        let programs = [
            ...
            (FunctionId(1), vec![Instruction::Return { value: 0 }]),
        ];
        let mut maximum_block_cost = 0;
        artifact.modules[0].blocks.clear();
        artifact.modules[0].code.clear();
...
pub(super) fn terminal_conformance_artifact(output: &[u16]) -> VerifiedArtifact {
    let string = ValueType {
        kind: 7,
        flags: 0,
        nominal_type: TypeId(0x8000_0000),
    ...
    literal_string_program_blocks_configured(
        ...
        vec![
            ...
            vec![
                Instruction::StringHash { dst: 2, string: 1 },
                Instruction::Return { value: 2 },
            ],
        ],
        |artifact| {
            ...
            artifact.manifest.required_capabilities = 1;
            artifact.manifest.maximum_host_requests = 3;
            let NominalType::Function { flags, .. } = &mut artifact.modules[0].types[0] else {
                unreachable!();
            };
            *flags = 1;
...
pub(crate) fn stdio_conformance_artifact(stderr: &[u16]) -> VerifiedArtifact {
    let string = ValueType {
        kind: 7,
        flags: 0,
        nominal_type: TypeId(0x8000_0000),
    ...
    literal_string_program_blocks_configured(
        ...
        vec![
            ...
            vec![
                ...
                Instruction::StringHash { dst: 2, string: 0 },
                Instruction::Return { value: 2 },
            ],
        ],
        |artifact| {
            ...
            let namespace_end = bytes.len();
            let name_start = bytes.len();
            bytes.extend_from_slice(b"stdio");
            let name_end = bytes.len();
            artifact.bytes = Arc::from(bytes);
            artifact.modules[0].strings[0] = ByteRange {
                start: namespace_start,
...
pub(crate) fn compiler_compile_artifact(source: &[u16], output: &[u16]) -> VerifiedArtifact {
    let string = ValueType {
        kind: 7,
        flags: 0,
        nominal_type: TypeId(0x8000_0000),
    ...
    literal_string_program_blocks_configured(
        ...
        |artifact| {
            ...
            let namespace_end = bytes.len();
            let name_start = bytes.len();
            bytes.extend_from_slice(b"compiler");
            let name_end = bytes.len();
            let source_start = bytes.len();
            for unit in source {
                bytes.extend_from_slice(&unit.to_le_bytes());
...
pub(crate) fn filesystem_recovery_reader_artifact() -> VerifiedArtifact {
    let string = ValueType {
        kind: 7,
        flags: 0,
        nominal_type: TypeId(0x8000_0000),
